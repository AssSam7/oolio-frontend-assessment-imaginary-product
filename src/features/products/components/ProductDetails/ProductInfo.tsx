...
const ProductInfo = ({ product }: ProductInfoProps) => {
  /*
   * Improvements:
   * - Removed unnecessary memoization around primitive product fields
   * - Added defensive fallback for optional discount values
   * - Simplified rating rendering logic
   * - Improved description rendering for multiline content
   * - Added safe guards around optional feature lists
   */
    ...
    <div className="space-y-4 md:space-y-6">
        ...
        <div className="flex items-center gap-1">
          {Array.from({ length: 5 }).map((_, i) => (
            <Icon
              ...
              size={16}
              color={
                i < ratingFloor ? "var(--color-warning)" : "var(--color-muted)"
              }
              className={i < ratingFloor ? "fill-warning" : "fill-muted"}
            />
          ))}
        </div>
      ...
      <div className="flex items-center gap-2">
        <Icon
          ...
          size={20}
          color={
            product.inStock ? "var(--color-success)" : "var(--color-error)"
          }
        />
